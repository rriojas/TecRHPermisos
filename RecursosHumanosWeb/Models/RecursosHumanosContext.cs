...
public partial class RecursosHumanosContext : DbContext
{
    ...
    // Constructor usado por DI when IHttpContextAccessor is available
    public RecursosHumanosContext(DbContextOptions<RecursosHumanosContext> options, IHttpContextAccessor httpContextAccessor)
        : base(options)
    {
    ...
    private void ApplyAuditInformation()
    {
        ...
        // Try to get current user id from HttpContext claims
        int currentUserId = 0;
        try
        {
            var user = _httpContextAccessor?.HttpContext?.User;
            var idClaim = user?.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (!string.IsNullOrEmpty(idClaim) && int.TryParse(idClaim, out var parsed))
            {
        ...
        foreach (var entry in ChangeTracker.Entries().Where(e => e.State == EntityState.Added || e.State == EntityState.Modified))
        {
            ...
            // FechaModificacion
            var propFechaMod = type.GetProperty("FechaModificacion");
            if (propFechaMod != null && propFechaMod.CanWrite)
            {
                propFechaMod.SetValue(entity, now);
            }

            if (entry.State == EntityState.Added)
            {
                // FechaCreacion
                var propFechaCre = type.GetProperty("FechaCreacion");
                if (propFechaCre != null && propFechaCre.CanWrite)
                {
                    var current = propFechaCre.GetValue(entity);
                    if (current == null || (current is DateTime dt && dt == default))
                    {
                        propFechaCre.SetValue(entity, now);
                    }
                }

                // Estatus default true
                var propEstatus = type.GetProperty("Estatus");
                if (propEstatus != null && propEstatus.CanWrite)
                {
                    var val = propEstatus.GetValue(entity);
                    if (val == null)
                    {
                        propEstatus.SetValue(entity, true);
                    }
                    else if (propEstatus.PropertyType == typeof(bool) && !(bool)val)
                    {
                        // keep explicit false, otherwise leave as is
                    }
                }

                // IdUsuarioCrea
                var propUsuarioCrea = type.GetProperty("IdUsuarioCrea");
                if (propUsuarioCrea != null && propUsuarioCrea.CanWrite && currentUserId > 0)
                {
                    var curr = propUsuarioCrea.GetValue(entity);
                    if (curr == null || (curr is int && (int)curr == 0))
                    {
                        propUsuarioCrea.SetValue(entity, currentUserId);
                    }
                }
            }

            // IdUsuarioModifica for both Added and Modified
            var propUsuarioMod = type.GetProperty("IdUsuarioModifica");
            if (propUsuarioMod != null && propUsuarioMod.CanWrite && currentUserId > 0)
            {
                propUsuarioMod.SetValue(entity, currentUserId);
            }
        }
    }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<ResetPasswordToken>(entity =>
        {
            ...
            entity.Property(e => e.IdUsuarioSolicita).IsRequired();
            ...
            entity.Property(e => e.FechaModificacion).HasColumnType("datetime");
            entity.Property(e => e.FechaCaducidad).HasColumnType("datetime").IsRequired();

            entity.HasIndex(e => e.Selector).IsUnique();
            entity.HasIndex(e => e.IdUsuarioSolicita);
            entity.HasIndex(e => e.FechaCaducidad);
        });
