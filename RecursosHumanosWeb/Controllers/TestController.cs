    ...
    public class TestController : ControllerBase
    {
        ...
        public async Task<IActionResult> RunCase(int @case = 1, bool confirmed = false)
        {
            ...
            if (@case == 1)
            {
                ...
                var fecha1 = corteActivo.Inicia.Value.AddHours(-1);
                if (fecha1 < corteActivo.Inicia.Value)
                {
            ...
            if (@case == 2)
            {
                ...
                var fecha1 = corteActivo.Inicia.Value.AddDays(1);
                var fecha2 = corteActivo.Termina.Value.AddDays(2);
                if (fecha2 > corteActivo.Termina.Value && !confirmed)
                {
                ...
                if (confirmed)
                {
                    var permiso = new Permiso
                    {
                        ...
                        Fecha1 = corteActivo.Inicia.Value.AddDays(1),
                        Fecha2 = corteActivo.Termina.Value.AddDays(2),
                        Dias = (int)((corteActivo.Termina.Value.AddDays(2) - corteActivo.Inicia.Value.AddDays(1)).TotalDays + 1),
            ...
            if (@case == 3)
            {
                ...
                var fecha1 = corteActivo.Inicia.Value.AddDays(1).Date;
                var fecha2 = corteActivo.Inicia.Value.AddDays(2).Date;
                var permiso = new Permiso
                {
